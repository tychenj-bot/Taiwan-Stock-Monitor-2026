...
# --- 1. 系統環境配置 ---
st.set_page_config(page_title="2026 戰略指揮中心 v10.1", layout="wide")
    ...
    def get_strategic_matrix(self, target_list, adr_premium):
        ...
        for stock_id, name, engine, driver in target_list:
            try:
                ...
                # 判定基準：統一使用法人成本
                cost_line = latest['Invest_Cost'] if "高息" in engine else latest['Foreign_Cost']
                ...
                results.append({
                    "引擎分類": engine, "標項名稱": name, "核心驅動": driver,
                    "今日開盤": f"${real_open:.1f}", "法人成本線": f"${cost_line:.1f}",
                    "狀態": status, "最終戰略指令": action
                })
            except: pass
...
st.sidebar.divider()
with st.sidebar.expander("📖 每日操作 SOP (已更新邏輯)", expanded=True):
    st.markdown("""
    **1️⃣ 15:30 (選股)**
    - 主力連買 >= 3 天 | RS 指標 > 0
    **2️⃣ 22:30 (定調)**
...
if not df.empty:
    ...
    with tab_open:
        ...
        m2.metric(f"防護線 ({cost_label})", f"${main_cost:.1f}")
        m3.metric("狀態", "守穩" if real_open > main_cost else "破線")
        if real_open > main_cost: st.success(f"✅ 開盤守穩 {cost_label}，參考上方矩陣指令。")
        else: st.error(f"🛑 跌破關鍵防線，取消交易。")
...
st.caption("v10.1 終極邏輯統一版：矩陣與細節均以法人成本為判定標準。")
