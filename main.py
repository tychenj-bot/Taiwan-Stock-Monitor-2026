...
class TaiwanStockCommander2026:
    ...
    def get_global_weather(_self):
        ...
        try:
            ...
            twd = yf.Ticker("TWD=X").history(period="2d") # 即時匯率 [微調 1]
    ...
    def get_strategic_data(_self, stock_id):
        ...
        df['RS_Index'] = (df['Close'].pct_change(20) - yf.Ticker("0050.TW").history(period=f"{days}d")['Close'].pct_change(20).values) * 100
...
for i, (tag, sid, sname) in enumerate(core_list):
    with cols[i]:
        df_c, fc, ic, rs_c = commander.get_strategic_data(sid)
        ...
        if adr_p > SYSTEM_CONFIG["ADR_THRESHOLD"]: st.warning("🔴 過熱")
        elif price_c > target_cost: st.success("🟢 守穩")
        else: st.error("🔴 破線")
...
with tab_open:
    ...
    with k1:
        st.metric("現價", f"${p_main:.1f}", delta=f"${p_main - m_cost:.1f}")
...
st.caption(f"{SYSTEM_CONFIG['VERSION']} | 以法人成本為核心 | 排除換新忘舊風險")
