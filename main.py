...
# --- 1. 系統環境配置 ---
st.set_page_config(page_title="2026 ADR 戰情系統 v6.1", layout="wide")
...
with col_main:
    ...
    st.metric("TSM ADR 溢價率 (總指揮)", f"{adr_premium:.2f}%", f"美股收盤 ${adr_price:.2f}", delta_color=delta_color)

with col_insight:
    if adr_premium > 5:
        st.warning("🔥 **過熱警戒**：溢價 > 5%，台股易開高走低，嚴禁追價。")
    elif adr_premium < -2:
        st.error("💎 **校正買點**：大幅負溢價。若下方個股籌碼不錯，今日開低為錯殺買點。")
    else:
        st.info("🟢 **多頭/盤整**：溢價正常，回歸個股基本面操作。")

st.divider()

# --- 次要監控：個股與 ETF 清單 (已更新) ---
st.markdown("### 🔍 標的驗證 (Secondary Checks)")

# 修正後的完整清單：個股只留台積電
targets = {
    "🔥 引擎一：成長進攻 (主動/半導體)": {
        "台積電 (2330) - 護國神山": "2330",
        "中信上游半導體 (00991A) - 設備戰隊": "00991A",
        "統一台股主動 (00981A)": "00981A", 
        "群益台灣精選 (00982A)": "00982A",
...
if not df.empty:
    ...
    if stock_id in ["0050", "006208"]:
        st.info("ℹ️ **基準標的**：此為大盤觀測基準。")
    elif adr_premium < -1 and con_buy > 0:
        st.success(f"🎯 **校正機會**：ADR 跌但內資/外資在買 {name}，留意開盤錯殺點。")
    elif bias < 2 and latest['Close'] > cost_src.iloc[-1]:
        st.success(f"✅ **順勢佈局**：股價守穩 {cost_name}，且未過熱。")
    else:
        st.warning("⚠️ **觀望/警戒**：無明確訊號或乖離過大。")
...
st.caption("v6.1 更新：個股聚焦台積電 (2330)，ETF 監控池完整補齊。")
